...
def cbs(pipeline_obj, ratio_file, temp_dir, bin_size, chromosomes):

    ratio_name = Path(ratio_file).stem.replace('_ratio', '')
    segments_file = Path(temp_dir) / f"{ratio_name}_segments.csv"
    temp_csv = prepare_cbs_data(ratio_file, ratio_name, temp_dir, bin_size, chromosomes)
...
def prepare_cbs_data(ratio_file, sample_name, temp_dir, bin_size, chromosomes):
