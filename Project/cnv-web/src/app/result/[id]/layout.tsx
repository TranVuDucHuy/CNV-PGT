...
const ResultDetailPageLayout: React.FC = () => {
  ...
  const handleExportWord = async () => {
    if (resultReport) {
      await exportToDocx(resultReport);
    }
    handleExportClose();
  };
  ...
  const handleExportPDF = async () => {
    if (resultReport) {
      await exportToPdf(resultReport);
    }
    handleExportClose();
  };
  ...
  return (
    ...
    >
      ...
      {!reportLoading && !reportError && resultReport && (
        <Box>
          <Box sx={{ ml: 1 }}>
            <Stack
              direction="row"
            ...
            >
              <Box>
                ...
                <Typography variant="body1">
                  {resultReport.sample.flowcell_id}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">
              ...
              <Box>
                ...
                <Typography variant="body1">
                  {resultReport.sample.reference_genome}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">
                ...
                <Typography variant="body1">
                  {new Date(resultReport.sample.date).toLocaleDateString()}
                </Typography>
              </Box>
            </Box>
          </Paper>
          ...
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
            ...
            >
              ...
              <Box>
                ...
                <Typography variant="body1">
                  {resultReport.algorithm.version}
                </Typography>
              </Box>
            </Box>

            {resultReport.algorithm.parameters.length > 0 && (
              <>
                ...
                <TableContainer>
                  <Table size="small">
                    ...
                    <TableBody>
                      {resultReport.algorithm.parameters.map((param, index) => (
                        <TableRow key={index}>
          ...
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
            ...
            {/* Aberration Summary */}
            {resultReport.aberration.aberration_summary &&
              resultReport.aberration.aberration_summary.length > 0 && (
                <Box sx={{ mb: 3 }}>
                  <Typography variant="subtitle2" gutterBottom>
                    Summary
                  </Typography>
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                    {resultReport.aberration.aberration_summary.map(
                      (summary, index) => (
                        <Chip
                          key={index}
                          label={summary}
            ...
            {/* Aberration Segments */}
            {resultReport.aberration.aberration_segments.length > 0 && (
              <>
                <Typography variant="subtitle2" gutterBottom sx={{ mt: 2 }}>
                  Aberration Segments (
                  {resultReport.aberration.aberration_segments.length})
                </Typography>
                <TableContainer>
                  <Table size="small">
                    <TableHead>
                    ...
                    <TableBody>
                      {resultReport.aberration.aberration_segments.map(
                        (segment, index) => (
                          <TableRow key={index}>
