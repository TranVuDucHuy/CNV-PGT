...
async def upload_file(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    SampleService.save(db=db, file_stream=io.BytesIO(content))
...
async def upload_multiple_files(files: List[UploadFile] = File(...), db: Session = Depends(get_db)):
    ...
    for file in files:
        content = await file.read()
        file_streams.append(io.BytesIO(content))
...
def get_sample_file(sample_id: str, db: Session = Depends(get_db)):
    ...
    file.seek(0)
    if file:
        return StreamingResponse(file, media_type="application/octet-stream", headers={"Content-Disposition": f"attachment; filename={sample_id}.bam"})
