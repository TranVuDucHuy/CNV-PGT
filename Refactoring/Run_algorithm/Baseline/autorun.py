...
def main():
    p = argparse.ArgumentParser(description="Automate Baseline runs for multiple test sets.")
    ...
    if not (project_root / "Code" / "baseline.py").exists():
        print(f"[ERROR] Could not find Code/baseline.py in {project_root}")
        sys.exit(1)
    ...
    for i, sub in enumerate(subs, start=1):
        ...
        print(f"  - Running baseline.py with -o {job_dir / 'Run'}")
        cmd = [sys.executable or "python3", str(project_root / "Code" / "baseline.py"), "-o", str(job_dir / "Run")]
        ...
        print("  - baseline.py run completed")
