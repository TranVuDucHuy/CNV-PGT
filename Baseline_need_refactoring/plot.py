...
# -*- coding: utf-8 -*-
"""
Module cho các chức năng plotting trong pipeline CNV
"""
...
class CNVPlotter:
    ...
    def __init__(self, chromosomes, binsize, output_dir):
        """
        Khởi tạo CNVPlotter
        
        Args:
            chromosomes (list): Danh sách chromosome cần phân tích
    ...
    def plot(self, ratio_npz, mean_filter_npz, segments_csv=None, pipeline_obj=None):
        """
        ...
        if pipeline_obj is not None:
            ...
            output_dir = pipeline_obj.output_dir
        ...
        if len(all_positions) == 0:
            ...
            if segments_df is not None:
                self._plot_segments(ax1, segments_df, ratio_data, binsize)
            
            # Vẽ đường giới hạn
            ax1.axhline(y=constitutional_3n, color='lightcoral', linestyle='--', alpha=0.8, linewidth=1.5, label='Constitutional 3n')
            ax1.axhline(y=constitutional_1n, color='lightblue', linestyle='--', alpha=0.8, linewidth=1.5, label='Constitutional 1n')
    ...
    def _plot_segments(self, ax, segments_df, ratio_data, binsize=None):
        """
        ...
        Args:
            ...
            binsize: Kích thước bin (nếu None sẽ dùng self.binsize)
        """
        if binsize is None:
            binsize = self.binsize
        ...
        for _, segment in segments_df.iterrows():
            ...
            if chrom in chrom_bin_mapping:
                ...
                end_maploc = segment['loc.end'] if 'loc.end' in segment.index else binsize
                
                # Tính bin index từ map location
                start_bin_idx = int(start_maploc // binsize)
                end_bin_idx = int(end_maploc // binsize)
    ...
    def plot_gccount_per_bin(self, gc_file, output_name="gccount_per_bin"):
        ...
        for chrom in self.chromosomes:
            if chrom in data.files:
                ...
                # Tính GC percentage
                gc_percentages = (gc_counts / self.binsize) * 100
    ...
    def create_additional_plots(self, pipeline_obj):
        ...
        try:
            gc_file = str(pipeline_obj.temp_dir / f"gc_content_binsize_{pipeline_obj.binsize}.npz")
        ...
        try:
            ...
            if case_raw_files and control_raw_files:
                ...
                for control_file in control_raw_files:
                    ratio_file = calculate_read_ratios(pipeline_obj, control_file, pipeline_obj.temp_dir / 'control_npz')
                    if ratio_file:
                ...
                print(f"Chuyển đổi được {len(control_ratio_files)} control ratio files và {len(case_ratio_files)} case ratio files")
                ...
                for case_ratio_file in case_ratio_files:
                    ...
                    if raw_ratio_file:
                        ...
                        plot_file = self.plot(raw_ratio_file, filtered_raw_mean_file, raw_segments_file, pipeline_obj=pipeline_obj)
                        
                        if plot_file:
                            # Đổi tên để phân biệt với normalized plot
                            new_name = Path(plot_file).stem.replace('_cnv_plot', '_cnv_plot_without_normalization') + '.png'
